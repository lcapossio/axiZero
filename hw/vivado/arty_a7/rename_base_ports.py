...
"""Rename SpinalHDL io_* ports in AxiZeroArtyDUT.v to AXI-standard names.

create_project.tcl references pin names like `axizero_0/s0_axi_awvalid` and
`axizero_0/aclk`/`axizero_0/aresetn`.  SpinalHDL natively emits raw bundle
names (`io_masters_0_aw_valid`, `io_masters_0_aw_payload_addr`, …) plus
`clk`/`resetn`.  Without this rename step the script-based Vivado flow
fails with "No pins matched" errors.

Idempotent: re-running on already-renamed Verilog is a no-op (the regex
only matches `io_masters_*` / `io_slaves_*`, which disappear after the
first pass).
"""
...
V_FILE = HERE / "ip" / "rtl" / "AxiZeroArtyDUT.v"
