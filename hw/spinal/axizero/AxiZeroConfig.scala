...
/** AXI3 master port. The crossbar auto-inserts an Axi3ToAxi4Adapter between the external AXI3 port
  * and the internal AXI4 fabric. Requires MasterPort.axi3Cfg to be set.
  */
...
/** QoS-based: uses AXQOS to rank requests and falls back to round-robin among equal-priority
  * requests. Masters without QoS signal are treated as AXQOS=0. Waiting requesters receive age
  * boost to prevent starvation.
  */
case object QosBased extends ArbitrationPolicy

/** Weighted round-robin: master i gets weights(i) grants per round. weights.size must equal the
  * number of masters.
  */
case class WeightedRoundRobin(weights: Seq[Int]) extends ArbitrationPolicy {
  require(
    weights.nonEmpty && weights.forall(_ > 0),
...
case class MasterPort(
  /** AXI bus parameters (address/data/id widths, optional signals). For Axi3Mode ports this
    * describes the internal AXI4 representation; the external port is an Axi3 bundle derived from
    * axi3Cfg.
    */
  config: Axi4Config,
  /** AXI4-Lite, full AXI4, or AXI3. */
  mode: PortMode = FullAxi4,
  /** Insert a register slice between this master and the crossbar fabric. Helps with timing
    * closure; costs one cycle of additional latency.
    */
...
case class AxiZeroConfig(
  ...
  /** Override the internal fabric data width. When None the fabric uses the maximum data width
    * across all ports; width converters are inserted at ports with a narrower config.
    */
  internalDataWidth: Option[Int] = None,
  /** Maximum outstanding transactions per slave per direction. 1 = blocking (simple, no FIFOs —
    * good for single-master or low-contention). >1 = non-blocking / pipelined (uses per-slave
...
object AxiZeroConfig {
  ...
  /** Build an all-AXI4-Lite config quickly (all ports share the same address and data widths).
    */
  def allLite(
    numMasters: Int,
    numSlaves: Int,
