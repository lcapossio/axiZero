...
class QosCrossbarSpec extends AnyFunSuite {
  ...
  test("qos: equal QoS uses round-robin tie-break") {
    simCfg.compile(new AxiZeroMixedTop(makeCfg(maxOutstanding = 1))).doSim { dut =>
      ...
      assert(
        r1Done0 < r1Done1,
        s"round 1 should prefer master 0 from reset RR pointer: m0=$r1Done0 m1=$r1Done1"
      )
      assert(
        r2Done0 < r2Done1,
        s"round 2: RR pointer wrapped to 0, master 0 wins first again: m0=$r2Done0 m1=$r2Done1"
      )
    }
  }
  ...
  test("qos: higher ARQOS wins on read arbitration") {
    simCfg.compile(new AxiZeroMixedTop(makeCfg(maxOutstanding = 1))).doSim { dut =>
      ...
      val f0 = fork {
        while (!go) cd.waitSampling()
        rd0 =
          SimHelpers.fullRead(dut.io.masters(0), cd, slaveBase.toLong + 0x400L, id = 3, qos = 1)._1
        done0 = simTime()
      }
      val f1 = fork {
        while (!go) cd.waitSampling()
        rd1 =
          SimHelpers.fullRead(dut.io.masters(1), cd, slaveBase.toLong + 0x404L, id = 4, qos = 13)._1
        done1 = simTime()
      }
  ...
  test("qos: aging prevents starvation of low-QoS requester") {
    simCfg.compile(new AxiZeroMixedTop(makeCfg(maxOutstanding = 1))).doSim { dut =>
      ...
      assert(
        doneLo < doneHi,
        s"low-QoS transaction should complete before high-QoS stream ends: lo=$doneLo hi=$doneHi"
      )
